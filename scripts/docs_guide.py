...
<p><code>std/json</code> provides high-performance JSON parsing, unified dot/index-path navigation (<code>"users[1].name"</code>, <code>"server.port"</code>), bracketed and escaped dot keys (<code>"[\"a.b\"]"</code>, <code>"a\\.b"</code>), 64-bit integer (<code>int64_t</code>) and <code>double</code> precision numeric getters, surrogate pair UTF-16 decoding (<code>\\uD83D\\uDE00</code>), complete RFC 8259 compliance, full structural validation with line/column diagnostics (<code>json_valid()</code>, <code>json_valid_ex()</code>), pretty-printing (<code>json_format_pretty()</code>), capped file loading (<code>json_load_file()</code>, <code>json_load_file_capped()</code>), and a container-enforced fluent <code>JsonBuilder</code>:</p>
{make_code_box("rook", """
#comprise <std/json>
...
<p><code>std/toml</code> provides a zero-allocation TOML v1.0 parser and serializer supporting sections, nested tables (<code>[a.b]</code>), array of tables (<code>[[table]]</code>), dotted keys (<code>a.b = 1</code>), single/double-quoted keys (<code>'my key' = 1</code>), inline tables (<code>{{ x = 1 }}</code>), 64-bit integers (<code>toml_get_i64</code>), IEEE 754 floats with special values (<code>inf</code>, <code>-inf</code>, <code>nan</code>), datetime validation and parsing (<code>toml_get_date_time</code>), string escapes, type introspection (<code>TomlType</code>), line/column key location (<code>toml_find_pos</code>), raw token extraction (<code>toml_get_raw</code>), capped file loading (<code>toml_load_file</code>, <code>toml_load_file_capped</code>), and a fluent <code>TomlBuilder</code> with string, literal, and array setters:</p>
{make_code_box("rook", """
#comprise <std/toml>
