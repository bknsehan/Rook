...
static void llvm2_backend_destroy(Backend* b) {
    free(b);
}

static void llvm2_backend_compile_and_link_raw_c(LLVMGen* g, Program* prog) {
    if (!g || !prog) return;
...
static LLVMModuleRef llvm2_backend_build_module(LLVMContextRef ctx, Sema* sema, Program* prog, int bounds_check, const char* target_triple) {
    ...
    llvm2_backend_compile_and_link_raw_c(&g, prog);
...
static char* llvm2_backend_emit_program(Sema* sema, Program* prog, int* out_len, int bounds_check) {
    return llvm2_backend_emit_program_target(sema, prog, out_len, bounds_check, NULL);
}
...
int llvm2_backend_emit_obj_target(Sema* sema, Program* prog, const char* obj_path, int opt_level, const char* target_triple) {
    LLVMContextRef ctx = LLVMContextCreate();
    LLVMModuleRef module = llvm2_backend_build_module(ctx, sema, prog, 0, target_triple);
    if (!module) {
        LLVMContextDispose(ctx);
        return 1;
    }
...
int llvm2_backend_emit_obj(Sema* sema, Program* prog, const char* obj_path, int opt_level) {
    return llvm2_backend_emit_obj_target(sema, prog, obj_path, opt_level, NULL);
}
...
int llvm2_backend_compile_ll_to_obj(const char* ll_path, const char* obj_path, int opt_level) {
    return llvm2_backend_compile_ll_to_obj_target(ll_path, obj_path, opt_level, NULL);
}
...
int llvm2_backend_jit_run(Sema* sema, Program* prog, int argc, char** argv) {
    ...
    LLVMContextRef ctx = LLVMContextCreate();
    LLVMModuleRef module = llvm2_backend_build_module(ctx, sema, prog, 0, NULL);
    if (!module) {
        LLVMContextDispose(ctx);
        return -1;
    }
...
Backend* llvm2_backend_create(void) {
    ...
    if (!b) return NULL;
    b->name = "llvm2";
    b->emit_program = llvm2_backend_emit_program;
    b->emit_program_target = llvm2_backend_emit_program_target;
    b->emit_obj = llvm2_backend_emit_obj;
    b->jit_run = llvm2_backend_jit_run;
    b->destroy = llvm2_backend_destroy;
    return b;
}
