...
static void usage(void) {
    ...
    printf("  rokade --emit-llvm2 <file> (alias for --emit-llvm)\n");
...
typedef struct {
    ...
    /* Dynamic library and include collections */
    char** libraries;
    size_t n_libraries;
    size_t cap_libraries;

    char** include_dirs;
    size_t n_include_dirs;
    size_t cap_include_dirs;
...
static void project_config_add_library(ProjectConfig* cfg, const char* lib) {
    if (!cfg || !lib || !lib[0]) return;
    for (size_t i = 0; i < cfg->n_libraries; i++) {
        if (strcmp(cfg->libraries[i], lib) == 0) return;
    }
    if (cfg->n_libraries >= cfg->cap_libraries) {
        size_t ncap = cfg->cap_libraries ? cfg->cap_libraries * 2 : 16;
        char** narr = (char**)realloc(cfg->libraries, ncap * sizeof(char*));
        if (!narr) return;
        cfg->libraries = narr;
        cfg->cap_libraries = ncap;
    }
    cfg->libraries[cfg->n_libraries++] = strdup(lib);
}

static void project_config_add_include_dir(ProjectConfig* cfg, const char* dir) {
    if (!cfg || !dir || !dir[0]) return;
    for (size_t i = 0; i < cfg->n_include_dirs; i++) {
        if (strcmp(cfg->include_dirs[i], dir) == 0) return;
    }
    if (cfg->n_include_dirs >= cfg->cap_include_dirs) {
        size_t ncap = cfg->cap_include_dirs ? cfg->cap_include_dirs * 2 : 16;
        char** narr = (char**)realloc(cfg->include_dirs, ncap * sizeof(char*));
        if (!narr) return;
        cfg->include_dirs = narr;
        cfg->cap_include_dirs = ncap;
    }
    cfg->include_dirs[cfg->n_include_dirs++] = strdup(dir);
}

static void project_config_add_pkg_config(ProjectConfig* cfg, const char* pkg) {
    if (!cfg || !pkg || !pkg[0]) return;
    for (size_t i = 0; i < cfg->n_pkg_config; i++) {
        if (strcmp(cfg->pkg_config[i], pkg) == 0) return;
    }
    if (cfg->n_pkg_config >= cfg->cap_pkg_config) {
        size_t ncap = cfg->cap_pkg_config ? cfg->cap_pkg_config * 2 : 8;
        char** narr = (char**)realloc(cfg->pkg_config, ncap * sizeof(char*));
        if (!narr) return;
        cfg->pkg_config = narr;
        cfg->cap_pkg_config = ncap;
    }
...
static void project_config_free(ProjectConfig* cfg) {
    ...
    for (size_t i = 0; i < cfg->n_include_dirs; i++) {
        free(cfg->include_dirs[i]);
    }
    free(cfg->include_dirs);
    cfg->include_dirs = NULL;
    cfg->n_include_dirs = 0;
    cfg->cap_include_dirs = 0;

    for (size_t i = 0; i < cfg->n_pkg_config; i++) {
        free(cfg->pkg_config[i]);
    }
    free(cfg->pkg_config);
    cfg->pkg_config = NULL;
    cfg->n_pkg_config = 0;
...
static int parse_toml_line(const char* line, ProjectConfig* cfg, char* cur_sec, size_t cur_sec_len) {
    ...
    if (*val == '"') {
        ...
        while (*cur && *cur != ']') {
            ...
            if (strcmp(key, "library") == 0 || strcmp(key, "libraries") == 0) {
                project_config_add_library(cfg, valbuf);
    ...
    if (strcmp(cur_sec, "package") == 0 || strcmp(cur_sec, "project") == 0 || cur_sec[0] == '\0') {
        if (strcmp(key, "name") == 0) snprintf(cfg->name, sizeof(cfg->name), "%s", valbuf);
            ...
            snprintf(cfg->backend, sizeof(cfg->backend), "%s", strcmp(valbuf, "llvm2") == 0 ? "llvm" : valbuf);
        }
    ...
    if (strcmp(cur_sec, "build") == 0 || strcmp(cur_sec, "project") == 0 || cur_sec[0] == '\0') {
        ...
        else if (strcmp(key, "library") == 0 || strcmp(key, "libraries") == 0) {
            project_config_add_library(cfg, valbuf);
        }
        else if (strcmp(key, "include-dir") == 0 || strcmp(key, "include-dirs") == 0) {
            project_config_add_include_dir(cfg, valbuf);
...
static void resolve_project_dependencies(const char* proj_dir, ProjectConfig* cfg, int depth) {
    ...
    for (size_t i = 0; i < cfg->n_dependencies; i++) {
        ...
        if (access(src_dir, R_OK) == 0) {
            project_config_add_include_dir(cfg, src_dir);
        }
        /* 2. Add <dep> itself to include_dirs */
        if (access(resolved_dir, R_OK) == 0) {
            project_config_add_include_dir(cfg, resolved_dir);
        }
...
static int do_build(const char* proj_path, const char* cli_target, const char* cli_backend, int build_all) {
    ...
    if (strcmp(active_backend, "llvm2") == 0) active_backend = "llvm";
    ...
    if (cfg.n_libraries > 0) {
        libs = (const char**)malloc(cfg.n_libraries * sizeof(const char*));
        if (libs) {
...
static int run_single_file_native(const char* path, const char* active_backend) {
    ...
    if (strcmp(active_backend, "llvm") == 0 || strcmp(active_backend, "llvm2") == 0) {
#ifdef ROKADE_HAS_LLVM
        int slen = 0;
        char* raw_src = util_read_file(path, &slen);
        if (!raw_src) {
            fprintf(stderr, "rokade: cannot read '%s'\n", path);
            rmdir(work);
            return 1;
        ...
        char* expanded = resolve_includes(raw_src, slen, basedir, NULL, 0, 0, path);
        free(raw_src);
        if (!expanded) {
            fprintf(stderr, "rokade: error resolving includes in '%s'\n", path);
            rmdir(work);
        ...
        char obj_path[4096];
        snprintf(obj_path, sizeof(obj_path), "%s/app.o", work);
        int emit_rc = llvm_backend_emit_obj(sema, p, obj_path, 2);
        sema_free(sema); program_free(p); free(expanded); free(toks);
        if (emit_rc != 0) {
            fprintf(stderr, "rokade: failed to emit object file\n");
            rmdir(work);
            return 1;
        }
        const char* link_args[] = { "clang", "-O2", obj_path, "-o", exe_path, "-lm", NULL };
        if (util_exec(link_args) != 0) {
            fprintf(stderr, "rokade: failed to link executable\n");
            unlink(obj_path);
            rmdir(work);
            return 1;
        }
        unlink(obj_path);
#else
        fprintf(stderr, "rokade: LLVM backend not enabled in this build\n");
        rmdir(work);
        return 1;
#endif
    } else {
        ...
        char c_path[4096];
        snprintf(c_path, sizeof(c_path), "%s/app.c", work);
        if (write_all(c_path, c, clen)) {
            free(c);
            rmdir(work);
            return 1;
        }
...
static int do_run(const char* proj_path, const char* cli_backend, int use_jit) {
    const char* default_proj_dir = ".";
    if (proj_path) {
        size_t plen = strlen(proj_path);
        if (plen < 5 || strcmp(proj_path + plen - 5, ".rook") != 0) {
            default_proj_dir = proj_path;
        }
    }
    ProjectConfig probe_cfg;
    const char* active_backend = "c";
    if (cli_backend && cli_backend[0]) {
        active_backend = cli_backend;
    } else if (use_jit) {
        active_backend = "llvm";
    } else if (read_project_config(default_proj_dir, &probe_cfg)) {
        if (probe_cfg.backend[0]) active_backend = probe_cfg.backend;
    ...
    if (proj_path) {
        ...
        if (plen >= 5 && strcmp(proj_path + plen - 5, ".rook") == 0) {
            if (use_jit) {
                if (strcmp(active_backend, "c") == 0) {
                    fprintf(stderr, "error: --jit is only supported with the llvm backend\n");
                    return 1;
                }
                return run_single_file_jit(proj_path, active_backend);
            }
            return run_single_file_native(proj_path, active_backend);
        }
    }
    if (use_jit) {
        if (strcmp(active_backend, "c") == 0) {
    ...
    if (read_project_config(proj_path ? proj_path : ".", &cfg)) {
        ...
        if (!cli_backend && cfg.backend[0]) active_backend = cfg.backend;
        if (strcmp(active_backend, "llvm2") == 0) active_backend = "llvm";
    } else {
        fprintf(stderr, "error: %s/rokade.toml not found or invalid\n", proj_path ? proj_path : ".");
        return 1;
    ...
    int ret = do_build(proj_path, run_target, active_backend, 0);
    if (ret != 0) {
        project_config_free(&cfg);
        return ret;
    }
...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "lsp") == 0 || strcmp(argv[1], "--lsp") == 0) {
    ...
    if (mode == 5) {
        ...
        for (int i = 0; i < parse_error_count(); i++) {
            ...
            if (!pd) continue;
            if (!first) sb_append(&jbuf, ",");
            first = 0;
            sb_append(&jbuf, "{\"file\":");
            json_append_str(&jbuf, pd->file[0] ? pd->file : path);
            sb_appendf(&jbuf, ", \"line\":%d, \"character\":%d, \"endCharacter\":%d, \"severity\":\"error\", \"code\":\"%s\", \"message\":",
                       pd->line, pd->col, pd->col + pd->len, pd->code);
            json_append_str(&jbuf, pd->msg);
            sb_append(&jbuf, "}");
        }
        if (!p) {
            if (parse_error_count() == 0 && parse_error()[0])
            ...
            c_import_program_raw(sema, p, NULL, 0);
            sema_collect(sema, p);
            sema_check_all(sema, p);
            lint_run_with_src(sema->prog, 0, sema, expanded);
            for (int i = 0; i < sema_diag_count(sema); i++) {
                SemaDiag* sd = sema_diag_get(sema, i);
                if (!sd) continue;
                if (!first) sb_append(&jbuf, ",");
                first = 0;
                const char* sev = sd->severity == 2 ? "warning" :
                                  sd->severity == 3 ? "info" :
                                  sd->severity == 4 ? "hint" : "error";
                sb_append(&jbuf, "{\"file\":");
                json_append_str(&jbuf, sd->file[0] ? sd->file : path);
                sb_appendf(&jbuf, ", \"line\":%d, \"character\":%d, \"endCharacter\":%d, \"severity\":\"%s\", \"code\":\"%s\", \"message\":",
                           sd->line, sd->col, sd->end_col, sev, sd->code);
                json_append_str(&jbuf, sd->message);
                sb_append(&jbuf, "}");
            }
            if (sema_diag_count(sema) == 0 && sema->err && sema->err[0])
                emit_diag_json(&jbuf, path, sema->err);
            sema_free(sema);
        }
        sb_append(&jbuf, "]");
    ...
    if (mode == 7) {
        ...
        int rc = llvm_backend_emit_obj(sema, p, out_obj, opt_level > 0 ? opt_level : 2);
        if (rc == 0) {
            printf("emitted: %s\n", out_obj);
        }
        sema_free(sema);
    ...
    if (mode == 1 || mode == 6 || mode == 8 || (mode == 0 && (strcmp(cli_backend, "llvm") == 0 || strcmp(cli_backend, "llvm2") == 0))) {
        ...
        char* c = NULL;
        if ((strcmp(bname, "llvm") == 0 || strcmp(bname, "llvm2") == 0) && opt_level > 0) {
            c = llvm_backend_emit_program_opt(sema, p, &elen, bounds_check, NULL, opt_level);
